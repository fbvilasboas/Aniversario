...
#pyinstaller --onefile -w --add-data "consulta.iqy;." --add-data "FELIZ.PNG;." aniversario_imagem_e_lista.py
...
# Define o tamanho da janela (800x500 pixels)
largura = 600
altura = 300
...
# Aplica o tamanho e a posição da janela
root.geometry(f"{largura}x{altura}+{pos_x}+{pos_y}")

root.resizable(False, False)  # Bloqueia redimensionamento horizontal e vertical
...
texto_explicativo = tk.Label(
    ...
    wraplength=500,  # Define a largura máxima do texto antes de quebrar a linha
    justify="center",  # Centraliza o texto
    font=("Arial", 14)   # Define a fonte e o tamanho do texto
)
...
# Cria um estilo para o Combobox
style = ttk.Style()
style.configure("TCombobox", font=("Verdana", 16))  # Altera a fonte do Combobox

# Cria o Combobox com largura personalizada
combobox_mes = ttk.Combobox(root, values=meses, state="readonly", width=20)  # Largura de 20 caracteres
...
style.configure("TButton",
                ...
                #background="#FFFF",         # Cor de fundo (verde)
                padding=10 ,                   # Espaçamento interno
                borderwidth=3,                # Largura da borda
                relief="relief"             # Estilo da borda (raised, sunken, flat, etc.)
                )

# Botão para buscar aniversariantes
btn_buscar = ttk.Button(root, text="ENVIAR LISTA", command=buscar_aniversariantes)
btn_buscar.configure(cursor="hand2")  # Muda o cursor para uma "mão"
